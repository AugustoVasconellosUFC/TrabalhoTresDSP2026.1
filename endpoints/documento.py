...
documentos_router = APIRouter(prefix="/documents", tags=["Documentos"])

@documentos_router.get("/{document_id}", response_model=DocumentMetadata)
async def obter_metadados_documento(document_id: str):
    """
    Passos (Requisito: GET /documents/{document_id}):
    1. Procura o documento pelo ID no MongoDB e devolve os metadados.
    """
    if not PydanticObjectId.is_valid(document_id):
        raise HTTPException(status_code=400, detail="ID inválido")
        
    doc = await DocumentMetadata.get(PydanticObjectId(document_id))
    if not doc:
...
async def download_documento(document_id: str):
    """
    Passos (Requisito: GET /documents/{document_id}/download):
    1. Recupera os metadados do MongoDB para descobrir a extensão e o content type.
...
@documentos_router.put("/{document_id}", response_model=DocumentMetadata)
async def substituir_documento(document_id: str, file: UploadFile = File(...)):
    """
    Passos (Requisito: PUT /documents/{document_id}):
...
@documentos_router.delete("/{document_id}", status_code=status.HTTP_204_NO_CONTENT)
async def eliminar_documento(document_id: str):
    """
    Passos (Requisito: DELETE /documents/{document_id}):
